...
function VerifyEmail() {
    ...
    const { resendEmail, loading } = useVerifyEmail(email)
    const openEmail = () => {
        if (!email) return
    ...
    return (
        <div className="relative bg-white h-screen px-4 py-10 overflow-hidden">
            <div className="flex flex-col items-center gap-10 max-w-[798px] py-10 mx-auto bg-gray-100 rounded-2xl">
                <h3 className="headline3 text-black lg:headline2">Check your email</h3>
                <Mail size={100} color="#2FAC8E" strokeWidth={2} />
                <p className="flex flex-col items-center gap-2">
                    <span className="body3 lg:body1">We’ve sent a verification link to your email.</span>
                    <span className="body3 lg:body1">Please check your inbox and verify your account.</span>
                </p>
                <Button variant="primary" size="lg" onClick={openEmail}>
                    Open Email
                </Button>
                <div className="flex items-center gap-2">
                    <span className="body3 lg:body2 text-black">Didn't receive the email?</span>
                    <Button
                        onClick={resendEmail}
                        disabled={loading}
                        variant="ghost"
